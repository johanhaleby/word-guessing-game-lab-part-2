...
import se.haleby.wordguessinggame.domain.readmodel.OngoingGameReadModel.Guess
...
internal class AssembleGameReadModelFromDomainEvents(val gameReadModel: GameReadModel? = null) {
    ...
    private fun applyEvent(e: PlayerWasAwardedPointsForGuessingTheRightWord): AssembleGameReadModelFromDomainEvents = e.run {
        val model = gameReadModel as GameWonReadModel
    ...
    private fun applyEvent(e: GameWasWon): AssembleGameReadModelFromDomainEvents = e.run {
        ...
        AssembleGameReadModelFromDomainEvents(GameWonReadModel(gameId, startedAt, timestamp, category, guesses.size, numberOfPlayersInGame, wordToGuess, winnerId, numberOfGuessesByWinner))
    }
    ...
    private fun applyEvent(e: GameWasLost): AssembleGameReadModelFromDomainEvents = e.run {
        ...
        AssembleGameReadModelFromDomainEvents(GameLostReadModel(gameId, startedAt, timestamp, category, guesses.size, numberOfPlayersInGame, wordToGuess))
    }
